...
using static AutoMapMarkers.Settings.MapMarkerConfig.Settings;

namespace AutoMapMarkers.Network
{
    /// <summary>
    ...
    public class MapMarkerNetwork
    {
        ...
        private void OnClientWaypointRequest(IPlayer fromPlayer, ClientWaypointRequest request)
        {
            ...
            WaypointUtil waypointUtil = new WaypointUtil(serverPlayer, MapMarkerConfig.GetSettings(CoreAPI, true));
            waypointUtil.AddWaypoint(request.waypointPosition, request.waypointSettings, request.sendChatMessageToPlayer, request.groupId, request.dynamicTitleComponent);
        }
        ...
        /// Side: client only
        /// </remarks>
        public void RequestWaypointFromServer(Vec3d position, AutoMapMarkerSetting settings, bool sendChatMessage, int groupId, string dynamicTitleComponent = null)
        {
            ...
            if (MapMarkerConfig.GetSettings(AutoMapMarkersModSystem.CoreAPI).DisableAllModFeatures)
            {
        ...
        private void ClientConnectivityCheck(object source = null, ElapsedEventArgs e = null)
        {
            ...
            else if (++ConnectionCheckAttempts > 9)
            {
                ...
                {
                    MessageUtil.Chat(Lang.Get("automapmarkers:server-warning"));
