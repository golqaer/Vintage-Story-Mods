...
namespace AutoMapMarkers.Events
{
    /// <summary>
    ...
    public class SneakHandler
    {
        ...
        public static void HandlePlayerSneak(EnumEntityAction action, bool on, ref EnumHandling handled)
        {
            ...
            MapMarkerConfig.Settings config = MapMarkerConfig.GetSettings(AutoMapMarkersModSystem.CoreAPI);
            if (config == null || config.EnableMarkOnSneak != true)
                return;

            BlockSelection blockSel = AutoMapMarkersModSystem.CoreClientAPI?.World?.Player?.CurrentBlockSelection;
            EntitySelection entitySel = AutoMapMarkersModSystem.CoreClientAPI?.World?.Player?.CurrentEntitySelection;
            ...
            //{
            //    Block blockBelow = AutoMapMarkersModSystem.CoreClientAPI.World.BlockAccessor.GetBlock(blockSel.Position.Copy().Down());
            ...
            if (thing.GetAssetPath().StartsWith("tallgrass-", StringComparison.Ordinal))
            {
                try
                {
                    ...
                    Block blockBelow = AutoMapMarkersModSystem.CoreClientAPI.World.BlockAccessor.GetBlock(downOnePos);
            ...
            AutoMapMarkersModSystem.Network.RequestWaypointFromServer(vecPos, settings, config.ChatNotifyOnWaypointCreation, config.ShareWaypointGroupId,thing.DynamicTitleComponent);
        }
    }
}
