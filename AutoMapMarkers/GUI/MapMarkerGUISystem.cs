    ...
    public class MapMarkerGUISystem : ModSystem
    {
        ...
        private void RegisterHotkeys()
        {
            ClientAPI.Input.RegisterHotKey(
                ...
                name: Lang.Get("automapmarkers:config-keybind-name"),
                key: GlKeys.M,
        ...
        private void RegisterCustomHotkeys()
        {
            if (!RegisteredCustomHotkeys)
            {
                ...
                string prefix = Lang.Get("automapmarkers:modname") + ": ";

                ClientAPI.Input.RegisterHotKey(
                    hotkeyCode: HotkeyCustom1,
                    name: prefix + Lang.Get("automapmarkers:config-keybind-custom1"),
                    key: GlKeys.Keypad1,
                    type: HotkeyType.GUIOrOtherControls,
                    altPressed: true, ctrlPressed: true, shiftPressed: false);
                ClientAPI.Input.RegisterHotKey(
                    hotkeyCode: HotkeyCustom2,
                    name: prefix + Lang.Get("automapmarkers:config-keybind-custom2"),
                    key: GlKeys.Keypad2,
                    type: HotkeyType.GUIOrOtherControls,
                    altPressed: true, ctrlPressed: true, shiftPressed: false);
                ClientAPI.Input.RegisterHotKey(
                    hotkeyCode: HotkeyCustom3,
                    name: prefix + Lang.Get("automapmarkers:config-keybind-custom3"),
                    key: GlKeys.Keypad3,
                    type: HotkeyType.GUIOrOtherControls,
                    altPressed: true, ctrlPressed: true, shiftPressed: false);
        ...
        private void RegisterDeleteHotkey()
        {
            if (!RegisteredDeleteHotkey)
            {
                ...
                ClientAPI.Input.RegisterHotKey(
                    ...
                    name: prefix + Lang.Get("automapmarkers:config-keybind-delete-nearest"),
                    key: GlKeys.Keypad0,
                    type: HotkeyType.GUIOrOtherControls,
                    altPressed: true, ctrlPressed: true, shiftPressed: false);
            ...
            try
            {
                ...
                if (settings.EnableCustomHotkeys)
                {
                    ...
                    if (customMarkerSettings.Enabled)
                    {
                        ...
                        if (pos != null)
                        {
                            AutoMapMarkersModSystem.Network.RequestWaypointFromServer(
                                position: pos.XYZ,
                                settings: customMarkerSettings,
                                sendChatMessage: settings.ChatNotifyOnWaypointCreation,
                                groupId: settings.ShareWaypointGroupId);
                        }
                    }
                }
            }
        ...
        public bool DeleteNearestMapMarker(KeyCombination keyCombo)
        {
            ...
            if (settings.EnableWaypointDeletionHotkey)
            {
                ...
                AutoMapMarkersModSystem.Network.RequestNearestWaypointDeletionFromServer(chatNotify);
            }
            else
            {
