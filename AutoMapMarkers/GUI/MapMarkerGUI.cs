...
using AutoMapMarkers.Utilities;

namespace AutoMapMarkers.GUI
{
    ...
    public class MapMarkerGUI : GuiDialog
    {
        ...
        public const string DialogID = "automapmarkers-config-menu";
        ...
        public readonly string ExtraSettingsTabName = Lang.Get("automapmarkers:ui");
        ...
        public override void OnGuiOpened()
        {
            ...
            {
                AutoMapMarkersModSystem.CoreAPI.Logger.Error("Map Marker Mod: Failed to initialize settings GUI. (" + e.ToString() + ")");
            }
            base.OnGuiOpened();
        }
        ...
        {
            CairoFont headerFont = CairoFont.WhiteSmallishText().Clone().WithWeight(FontWeight.Bold);
            ...
            string customTabName = Lang.Get("automapmarkers:custom");
            ...
            double opPromptWidth = 300;
            double toggleWidth = 100; //Toggle switch, plus empty space after
            double disabledMsgWidth = 200;
            double iconLabelWidth = GetFontTextWidth(CairoFont.WhiteSmallishText(), Lang.Get("automapmarkers:icon"), 12.0);  // Important to calculate these for localization
            double iconDropdownWidth = 60;
            double iconDropdownAfter = 28;
            double colorLabelWidth = GetFontTextWidth(CairoFont.WhiteSmallishText(), Lang.Get("automapmarkers:color"), 12.0);
            double colorInputWidth = 140;
            double colorInputAfter = 10;
            double colorPreviewWidth = 55; //Color preview box, plus empty space after
            double nameLabelWidth = GetFontTextWidth(CairoFont.WhiteSmallishText(), Lang.Get("automapmarkers:name"), 12.0);
            ...
            SingleComposer = capi.Gui.CreateCompo(DialogID, dialogBounds)
                ...
                .AddDialogTitleBar(Lang.Get("automapmarkers:config-menu-title"), OnTitleBarCloseClicked)
            ...
            {
                string enableLabel = Lang.Get("automapmarkers:re-enable-mod");
                SingleComposer
            ...
            if (needsPages)
            {
                ...
                {
                    string pageName = $"{Lang.Get("automapmarkers:page")} {p}";
                    CairoFont buttonFont = CairoFont.ButtonText();
                    int pVal = p;
                    SingleComposer.AddToggleButton(
                        text: pageName,
            ...
            foreach (var settingGroup in AutoMapMarkerSettings)
            {
                ...
                if (CurrentTab == customTabName)
                {
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:custom-gui-description"),
                ...
                foreach (var setting in settingGroup.Value)
                {
                    ...
                    .AddIf(markerSetting.Enabled == true)
                        ...
                        // Map marker icon
                        .AddStaticText(
                            text: Lang.Get("automapmarkers:icon"),
                            font: CairoFont.WhiteSmallishText(),
                        ...
                        // Map marker color
                        .AddStaticText(
                            text: Lang.Get("automapmarkers:color"),
                            font: CairoFont.WhiteSmallishText(),
                        ...
                        // Map marker name
                        .AddStaticText(
                            text: Lang.Get("automapmarkers:name"),
                            font: CairoFont.WhiteSmallishText(),
                    ...
                    .AddIf(markerSetting.Enabled == false)
                        ...
                        //"Disabled" message
                        .AddStaticText(
                            text: Lang.Get("automapmarkers:disabled"),
                            font: disabledFont,
                ...
                if (CurrentTab == Lang.Get("automapmarkers:traders"))
                {
                    ...
                    string buttonLabel = Lang.Get("automapmarkers:copy-setting-to-all-traders", firstTraderSettingName);
            ...
            if (CurrentTab == ExtraSettingsTabName)
            {
                ...
                ElementBounds groupDropdownBounds = ElementBounds.Fixed(0, 0, 200, rowHeight*0.75);
                ElementBounds groupLabelBounds = ElementBounds.Fixed(220, 2.5, 800, rowHeight);
                int hotkeyTooltipWidth = (int)(GetFontTextWidth(CairoFont.WhiteSmallText(), Lang.Get("automapmarkers:show-chat-message-hotkey-tooltip")) / 2 + 25);
                string hotkeyTooltipText = Lang.Get("automapmarkers:show-chat-message-hotkey-tooltip").Replace(">", "&gt;");

                var groups = AutoMapMarkersModSystem.CoreClientAPI.World.Player.Groups.Select((g, i) => new
                {
                    Index = i+1,
                    GroupId = g.GroupUid,
                    g.GroupName,
                ...
                groups.Add(new 
                {
                    Index = 0,
                    GroupId = -1,
                    GroupName = $"{Lang.Get("automapmarkers:noshare")}"                    
                });
                groups = groups.OrderBy(g => g.GroupId).ToList();

                int selectedIndex = groups
                    .Where(g => g.GroupId == ModSettings.ShareWaypointGroupId)
                    .Select(g => g.Index)
                    .FirstOrDefault();

                SingleComposer
                .BeginChildElements(markerOptionRowBounds)
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:mark-style-interact"),
                        font: CairoFont.WhiteSmallishText(),
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:mark-style-sneak"),
                        font: CairoFont.WhiteSmallishText(),
                        bounds: uiToggleLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:show-chat-message"),
                        font: CairoFont.WhiteSmallishText(),
                        bounds: uiToggleLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:enable-custom-hotkeys"),
                        font: CairoFont.WhiteSmallishText(),
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:enable-delete-hotkey"),
                        font: CairoFont.WhiteSmallishText(),
                        bounds: uiToggleLabelBounds = uiToggleLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))
                    .AddHoverText(
                        text: hotkeyTooltipText,
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    .AddDropDown(
                        values: groups.Select(g => g.GroupId.ToString()).ToArray(),
                        names: groups.Select(g => g.GroupName).ToArray(),
                        selectedIndex: selectedIndex,
                        bounds: groupDropdownBounds.FlatCopy().WithParent(markerOptionRowBounds),
                        onSelectionChanged: (string code, bool selected) => 
                        {
                            ModSettings.ShareWaypointGroupId = int.Parse(code);
                            SetupDialog();
                        })
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:noshare-desc"),
                        font: CairoFont.WhiteSmallishText(),
                        bounds: groupLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))
                .EndChildElements();

                if (ModSettings.EnableWaypointDeletionHotkey)
                {
                    SingleComposer
                    .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy().WithFixedOffset(0, 15))
                        ...
                        .AddStaticText(
                            text: Lang.Get("automapmarkers:show-chat-message-on-delete"),
                            font: CairoFont.WhiteSmallishText(),
                            bounds: uiToggleLabelBounds = uiToggleLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))

                    .EndChildElements();
                    markerOptionRowBounds = markerOptionRowBounds.FlatCopy();
                }
                ...
                .BeginChildElements(markerOptionRowBounds = markerOptionRowBounds.BelowCopy())
                    ...
                    .AddStaticText(
                        text: Lang.Get("automapmarkers:disable-mod"),
                        font: CairoFont.WhiteSmallishText(),
                        bounds: uiToggleLabelBounds = uiToggleLabelBounds.FlatCopy().WithParent(markerOptionRowBounds))
        ...
        public override void OnGuiClosed()
        {
            ...
            if (saveButton != null && saveButton.Enabled == false)
            {
                ...
                MessageUtil.Chat(Lang.Get("automapmarkers:not-saved-warning"));
            }
            else
            {
        ...
        private double GetFontTextWidth(CairoFont font, string text, double padding = 15.0)
        {
            ...
            return textExtents.Width / RuntimeEnv.GUIScale + padding;
        }
    }
}
